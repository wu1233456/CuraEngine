...
void InsetOrderOptimizer::processHoleInsets()
{
    ...
    if (!outer_inset_first && mesh.settings.get<bool>("infill_before_walls"))//true
    {
    ...
    for (unsigned int outer_poly_order_idx = 0; outer_poly_order_idx < order_optimizer.polyOrder.size(); ++outer_poly_order_idx)
    {
        ...
        {
            //for (int i = 0; i < Modify_outer_wall;i++) {
            //    // find the adjacent poly in the level 1 insets that encloses the hole
            //    int adjacent_enclosing_poly_idx = findAdjacentEnclosingPoly(hole_outer_wall[i], inset_polys[i+Modify_outer_wall], max_gap);
            ...
            //            Polygons inset;
            //            inset.add(*inset_polys[i+Modify_outer_wall][adjacent_enclosing_poly_idx]);
            ...
            //            if (PolygonUtils::polygonOutlinesAdjacent(inset[i], *inset_polys[i][0], max_gap))
            //            {
            ...
            //                if (PolygonUtils::polygonOutlinesAdjacent(inset[i], *inset_polys[i][outline_index], max_gap) ||
            //                    PolygonUtils::polygonOutlinesAdjacent(*inset_polys[i][outline_index], inset[i], max_gap))
            //                {
            ...
            //        }
            //    }
            //    else if (!outer_inset_first)//outer_inset_first false
            //    {
            ...
            //        PolygonUtils::findAdjacentPolygons(hole_level_1_wall_indices, hole_outer_wall[i], inset_polys[i+Modify_outer_wall], max_gap);
            //    }
            //}
            //// find the adjacent poly in the level 1 insets that encloses the hole
        ...
        for (unsigned int level_1_wall_idx = 0; level_1_wall_idx < hole_level_1_wall_indices.size(); ++level_1_wall_idx)
        {
            ...
            ConstPolygonPointer last_inset;
            
            last_inset = inset_polys[1][inset_idx];
            if (Modify_outer_wall>1) {
                hole_outer_wall.add(ConstPolygonRef(*last_inset));
            }
            else { hole_inner_walls.add(ConstPolygonRef(*last_inset)); }
            //hole_inner_walls.add(ConstPolygonRef(*last_inset));
            /*ConstPolygonPointer last_inset = inset_polys[Modify_outer_wall][inset_idx];
            hole_inner_walls.add(ConstPolygonRef(*last_inset));
            inset_polys[Modify_outer_wall].erase(inset_polys[Modify_outer_wall].begin() + inset_idx);*/
            ...
            for (unsigned int inset_level =2; inset_level < num_insets && inset_polys[inset_level].size(); ++inset_level)
            {
                ...
                if (i >= 0)
                {
                    ...
                    if (inset_level<Modify_outer_wall) {
                        hole_outer_wall.add(ConstPolygonRef(*last_inset));
                    }
                    else { hole_inner_walls.add(ConstPolygonRef(*last_inset)); }
        ...
        else if (extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr)
        {
            ...
            {
                gcode_layer.addWalls(hole_outer_wall, mesh, mesh_config.inset0_config, mesh_config.bridge_inset0_config, wall_overlapper_0);
            }
            // move inside so an immediately following retract doesn't occur on the outer wall
            moveInside();
            added_something = true;
...
{
    const coord_t wall_line_width_x = mesh_config.insetX_config.getLineWidth();
    const coord_t wall_0_wipe_dist = mesh.settings.get<coord_t>("wall_0_wipe_dist");//false
    const bool retract_before_outer_wall = mesh.settings.get<bool>("travel_retract_before_outer_wall");
    const bool outer_inset_first = mesh.settings.get<bool>("outer_inset_first")
                                    || (layer_nr == 0 && mesh.settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::BRIM);
    ...
    if (include_inners)
    {
        ...
        for (unsigned int level_1_wall_idx = 0; inset_polys.size() > 1 && level_1_wall_idx < inset_polys[Modify_outer_wall].size(); ++level_1_wall_idx)
        {
            ConstPolygonRef inner = *inset_polys[Modify_outer_wall][level_1_wall_idx];
            if (PolygonUtils::polygonsIntersect(inner, outer_wall))
            {
                ++num_level_1_insets;
                part_inner_walls.add(inner);
                // consume the level 1 inset
                inset_polys[Modify_outer_wall].erase(inset_polys[Modify_outer_wall].begin() + level_1_wall_idx);
                ...
                for (unsigned int inset_level = Modify_outer_wall+1; inset_level < num_insets && inset_polys[inset_level].size(); ++inset_level)
                {
    ...
    if (part_inner_walls.size() > 0 && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr)
    {
        ...
        else
        {
            ...
            if (num_level_1_insets == Modify_outer_wall)
            {
    ...
    else if (include_outer && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr)
    {
        ...
        int size = inset_polys.size();
        for (int i = 0; i < Modify_outer_wall; i++) {
            part_outer_wall.add(*inset_polys[i][0]);
...
bool InsetOrderOptimizer::processInsetsWithOptimizedOrdering()
{
    ...
    int Modify_outer_wall = mesh.settings.get<size_t>("wall_line_count_0");
    Polygons wall_0_polys;
    if (mesh.settings.get<bool>("travel_compensate_overlapping_walls_0_enabled"))//true
    {
        //wall_0_polys = part.insets[0];
        for (unsigned int inset_level = 0; inset_level < Modify_outer_wall; ++inset_level)
        {
            wall_0_polys.add(part.insets[inset_level]);
        }
    ...
    {
        for (unsigned int inset_level = Modify_outer_wall; inset_level < num_insets; ++inset_level)
        {
    ...
    //ÐÞ¸Ä
    unsigned int wall_0_polys_index = 0;
    for (unsigned int inset_level = 0; inset_level < Modify_outer_wall; ++inset_level) {
        inset_polys.emplace_back();
        for (unsigned int poly_idx = 0; poly_idx < part.insets[inset_level].size(); ++poly_idx)
        {
            ...
            {
                inset_polys[inset_level].push_back(wall_0_polys[wall_0_polys_index++]);
            }
            else
            {
   ...
   /* for (unsigned int poly_idx = 0; poly_idx < part.insets[0].size(); ++poly_idx)
    ...
    {
        Polygons remaining;
        for (unsigned int inset_level = Modify_outer_wall; inset_level < inset_polys.size(); ++inset_level)
        {
