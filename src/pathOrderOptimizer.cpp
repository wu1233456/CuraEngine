...
void PathOrderOptimizer::optimize()
    {
        ...
        for (unsigned poly_idx = 0; poly_idx < polygons.size(); ++poly_idx) /// find closest point to initial starting point within each polygon +initialize picked
        {
            ...
            default:
                ...
                //polyStart.push_back(0);//蔚晟楠改于2021-04-06
                break;
            }
            assert(poly.size() != 2);
            //assert(int expression)的含义即计算expression的值，若为假flase或0就停止程序运行
        }
        //按addPolygon -> 0.1.2顺序获得每个偏置多边形的最佳打印起点
        ...
        for (unsigned int poly_order_idx = 0; poly_order_idx < polygons.size(); poly_order_idx++) /// actual path order optimizer
        {
            ...
            //numeric_limits表示C++中的数值极限，infinity()得到无穷大值，类似于初始化时999.0f，学到了。
            ...
            if (best_poly_idx > -1) /// should always be true; we should have been able to identify the best next polygon
            {
                ...
                //Cura中认为polyStart离得最近的点是下一个应当打印的多边形
            }
            else
            {
...
void PathOrderOptimizer::optimizeNew()
{
    //蔚晟楠修改于2021-04-07
    // NOTE: Keep this vector fixed-size, it replaces an (non-standard, sized at runtime) array:
    std::vector<bool> picked(polygons.size(), false);
    ...
    //改后的优化器
    int best_poly_idx = -1;
    float bestDist2 = std::numeric_limits<float>::infinity();//初始化所有变量
    for (unsigned int poly_idx = 0; poly_idx < polygons.size(); poly_idx++)
    {
        if (picked[poly_idx] || polygons[poly_idx]->size() < 1) /// skip single-point-polygons
        {
        ...
        assert(polygons[poly_idx]->size() != 2);
        for (unsigned int j = 0; j < polygons[poly_idx]->size();j++) 
        {
            const Point& p = (*polygons[poly_idx])[polyStart[poly_idx]];
            float dist2 = vSize2f(p - prev_point);
            if (dist2 < bestDist2)
    ...
    {
        logError("Failed to find next closest polygon.\n");
    }//单独拎出来写是为了度过startPoint无索引这关
    //从第二个开始
    int g = best_poly_idx;
    Point pt1 = (*polygons[g])[polyStart[g]];
    Point pt2 = (*polygons[g])[(polyStart[g] + 1) % polygons[g]->size()];//指定两个点
    int temp;
    for (unsigned int poly_order_idx = 1; poly_order_idx < polygons.size(); poly_order_idx++) //从第二个开始
    {
        int f = -1;
        float bestDist2 = std::numeric_limits<float>::infinity();//初始化所有变量
        if (poly_order_idx != 1) 
        {
            Point pt1 = (*polygons[temp])[polyStart[temp]];
            Point pt2 = (*polygons[temp])[(polyStart[temp] + 1) % polygons[temp]->size()];//指定两个点
        }
        for (unsigned int poly_idx = 0; poly_idx < polygons.size(); poly_idx++)
        {
            if (picked[poly_idx] || polygons[poly_idx]->size() < 1) /// skip single-point-polygons
            ...
            for (unsigned int j = 0; j < polygons[poly_idx]->size(); j++)
            {
                Point pt3 = (*polygons[poly_idx])[j % polygons[poly_idx]->size()];
                Point pt4 = (*polygons[poly_idx])[(j + 1) % polygons[poly_idx]->size()];//指定两个点
                bool is = IsParallel(pt1, pt2, pt3, pt4);
                float dist2;
                if (is)
                {
                    dist2 = dist(pt1, pt2, pt3, pt4);
                    if (dist2 < bestDist2)
                    {
                        f = poly_idx;
                        temp = poly_idx;
        ...
        if (f > -1) /// should always be true; we should have been able to identify the best next polygon
        {
            assert(polygons[f]->size() != 2);
            prev_point = (*polygons[f])[polyStart[f]];
            picked[f] = true;
            polyOrder.push_back(f);
            //Cura中认为polyStart离得最近的点是下一个应当打印的多边形
        }
        else
        {
...
void PathOrderOptimizer::optimizeNewNew()
{
    startPoint = (*polygons[0])[1];
    // NOTE: Keep this vector fixed-size, it replaces an (non-standard, sized at runtime) array:
    std::vector<bool> picked(polygons.size(), false);
    loc_to_line = nullptr;
    ...
    for (unsigned poly_idx = 0; poly_idx < polygons.size(); poly_idx++) /// find closest point to initial starting point within each polygon +initialize picked
    {
        ...
        const ConstPolygonRef poly = *polygons[poly_idx];
        for(unsigned int poly_idx1 = 0; poly_idx1 < poly.size();poly_idx1++)
        {
            float dist = vSize2f(poly[poly_idx1] - startPoint);
            if (dist < bestdist)
            {
                best = poly_idx1;
                //best - poly_idx1;
...
bool PathOrderOptimizer::IsParallel(Point a, Point b, Point c, Point d)
{
    float k1,k2;
    if (b.X == a.X || c.X == d.X)
    {
    ...
    {
        //k1 = (float)(b.Y - a.Y) / (float)(b.X - a.X);
        //k2 = (float)(d.Y - c.Y) / (float)(d.X - c.X);
        //k1 = (double)(b.Y - a.Y) / (double)(b.X - a.X);
        //k2 = (double)(d.Y - c.Y) / (double)(d.X - c.X);
        //k1 = (double)((b.Y - a.Y) / (b.X - a.X));
        //k2 = (double)((d.Y - c.Y) / (d.X - c.X));
        k1 = 1.0f * (b.Y - a.Y) / (b.X - a.X);
        k2 = 1.0f * (d.Y - c.Y) / (d.X - c.X);
        if (k1 == k2)
        {
            return true;
        }
...
float PathOrderOptimizer::dist(Point a, Point b, Point c, Point d)
{
    float k;
    float distance;
    if (b.X == a.X)
    {
        distance = abs(c.X - a.X);
    }
    else 
    {
        //k = (double)(b.Y - a.Y) / (double)(b.X - a.X);
        k = 1.0 * (b.Y - a.Y) / (b.X - a.X);
        distance =  abs(k * c.X - c.Y - k * a.X + a.Y) / sqrt(1 + k * k);
    }
    return distance;
}
