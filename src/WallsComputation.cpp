...
void WallsComputation::generateInsets(SliceLayerPart* part)
{
    ...
    for(size_t i = 0; i < inset_count; i++)
    {
        ...
        if (i == 0 )
        {
            part->insets[i] = part->outline.offset(-line_width_0 / 2 - wall_0_inset);
        }
        else if (i == 1)
        {
            part->insets[i] = part->insets[0].offset(-line_width_0 / 2 + wall_0_inset - line_width_x / 2);
        }
        else
        {
